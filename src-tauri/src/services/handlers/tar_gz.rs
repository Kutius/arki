...
impl ArchiveHandler for TarGzHandler {
    fn list(&self, path: &Path) -> Result<ArchiveInfo, String> {
            ...
            .map_err(|e| format!("Failed to read TAR entries: {}", e))?
        {
            ...
            entries.push(ArchiveEntry {
                ...
                is_directory: is_dir,
                size,
                compressed_size: None, // TAR doesn't store per-file compressed size
                modified,
            });
    ...
    fn extract(
        ...
        // TAR is stream-based; totals are unknown until the full archive is read.
        // We report per-chunk progress with total_files/total_bytes = 0 (unknown).
            ...
            .map_err(|e| format!("Failed to read TAR entries: {}", e))?
        {
            ...
            // Extract header info before using entry for reading
            let entry_path;
            let entry_size;
            let is_dir;
            {
            ...
            if is_dir {
                ...
                chunked_copy(
                    ...
                    &entry_path,
                    files_processed,
                    0, // TAR: total_files unknown
                    bytes_processed,
                    0, // TAR: total_bytes unknown
                )?;
            }
    ...
    fn create(
        ...
        let level = Compression::new(options.compression_level as u32);
        let encoder = GzEncoder::new(buf_writer, level);
        ...
        encoder
            .finish()
            .map_err(|e| format!("Failed to finalize GZ compression: {}", e))?;
